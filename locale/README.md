...
```bash
podman compose -f local.yml run --rm django python manage.py makemessages -all --no-location
```
...
```bash
podman compose -f local.yml run --rm django python manage.py compilemessages
```
